...
# Usage
# ────────────────────────────────────────────────────────────────
#   ./setup_nvim_v19.sh [-b|--backup] [--light] [-t THEME] [--install-tools]
#
...
function _helpout() {
  if _has helpout; then helpout; return; fi
  if _has batwrap; then batwrap; return; fi
  if _has bat; then
...
function usage() {
  cat <<'EOF' | _helpout
# `setup_nvim_v19` — bootstrap a modern Neovim config (safe headless installs)

**Synopsis**
- `setup_nvim_v19.sh [-b|--backup] [--light] [-t THEME] [--install-tools]`
- `setup_nvim_v19.sh -h | --help`

**Options**
- `-b, --backup` — Backup existing `init.vim` and `plug.vim`
- `--light` — Use Gruvbox **light** variant
- `-t, --theme THEME` — Override colourscheme (default: `gruvbox`)
- `--install-tools` — Install Mason tools headlessly (shfmt, shellcheck, bash-language-server, pyright, lua-language-server, ruff-lsp)
- `-h, --help` — Show this help
...
function create_config() {
  ...
  cat > "$INIT_VIM" << 'EOF'
"=====================================================================
"  init.vim  —  generated by setup_nvim_v19.sh
"=====================================================================

" ── Headless bootstrap guard (skip system/UI plugins) ─────────────
if exists("$NVIM_BOOTSTRAP") && $NVIM_BOOTSTRAP ==# "1"
  " Quarantine system-wide vimfiles during headless installs
...
" ► UI-heavy plugins — keep OPT via fake filetype so they are not sourced during PlugInstall
"     They will be packadd()'d explicitly in interactive sessions.
Plug 'akinsho/bufferline.nvim', { 'tag': '*', 'for': ['_gxnever_'] }
Plug 'goolord/alpha-nvim',      { 'for': ['_gxnever_'] }
Plug 'folke/noice.nvim',        { 'for': ['_gxnever_'] }
Plug 'nvim-tree/nvim-tree.lua', { 'for': ['_gxnever_'] }
...
--------------------------------------------------------------------
-- Ensure optional UI plugins are loaded in interactive sessions
--------------------------------------------------------------------
...
-- Colourscheme
--------------------------------------------------------------------
local variant   = vim.env.GX_VARIANT or 'dark'
...
# Clean stale layouts so previously "start" placed plugins are removed.
echo "→ Cleaning unmanaged plugins (headless)…"
NVIM_BOOTSTRAP=1 nvim --headless +'PlugClean! --sync' +qa || true
