...
# • Telescope + FZF-native for fast search
# • LSP, autocompletion, snippets (Mason • nvim-cmp • LuaSnip)
# • Git integration (gitsigns + fugitive)
# • Status-/buffer-lines, dashboard, which-key, todo-comments, autopairs
# • File explorer (nvim-tree), UI (noice/trouble)
# • NEW: Conform (Black on save), Ruff LSP diagnostics, Python 100-col guide
#
# Usage
# ────────────────────────────────────────────────────────────────
#   ./setup_nvim_v17.sh [-b|--backup] [--light] [-t THEME]
#
...
#######################
usage() {
  sed -n '27,57p' "$0" # print the block above (help section)
  exit 0
}
...
create_config() {
  ...
  cat >"$INIT_VIM" <<'EOF'
"=====================================================================
"  init.vim  —  generated by setup_nvim_v17.sh
"=====================================================================
...
" ► Formatting & Linting (NEW)
Plug 'stevearc/conform.nvim'           " formatter runner (Black)
...
--------------------------------------------------------------------
-- Formatting (Conform) + Ruff LSP diagnostics (NEW) ---------------
--------------------------------------------------------------------
require('conform').setup({
  formatters_by_ft = {
    python = { 'black' },    -- use Black (respect pyproject.toml)
  },
...
-- Ruff LSP provides diagnostics and code actions; we keep formatting to Conform/Black
...
" ── Python textwidth & column guide (NEW) ────────────────────────
