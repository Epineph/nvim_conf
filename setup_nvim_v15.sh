...
# Usage
# ────────────────────────────────────────────────────────────────
#   ./setup_nvim_v13.sh [-b|--backup] [--light] [-t THEME]
#
...
#######################
usage() {
  sed -n '27,51p' "$0"  # print the block above (help section)
  exit 0
}
...
create_config() {
  ...
  cat > "$INIT_VIM" << 'EOF'
"=====================================================================
"  init.vim  —  generated by setup_nvim_v13.sh
"=====================================================================
...
require('lualine').setup {
  options = { theme = 'gruvbox', icons_enabled = true, section_separators = '', component_separ
ators = '' }
}
...
" ── Neoformat settings ──────────────────────────────────────────
" Format file on save if a formatter exists
...
" ── Neomake settings ────────────────────────────────────────────
" Run linting on save and on file open
augroup NeomakeAuto
  autocmd!
...
install_formatters() {
  ...
  if command -v micromamba >/dev/null 2>&1; then
    ...
    micromamba run -n pyEnv pip install --upgrade black
  else
